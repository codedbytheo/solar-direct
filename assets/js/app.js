...
function updateNavMenuStyles() {
	if (window.innerWidth <= 374) {
		...
		if (mobileMenu.classList.contains("active")) {
			if (window.location.href.includes("form.html")) {
				navMenu.innerHTML = 'Close<ion-icon name="close-outline"></ion-icon>';
			} else {
				navMenu.innerHTML = 'Close<ion-icon name="close-outline"></ion-icon>';
			}
		} else {
			if (window.location.href.includes("form.html")) {
				navMenu.innerHTML = 'Menu<ion-icon name="menu-outline"></ion-icon>';
			} else {
				navMenu.innerHTML = 'Contact<ion-icon name="open-outline"></ion-icon>';
			}
		}
	}
}
...
document.addEventListener("DOMContentLoaded", function () {
	...
	function checkScroll() {
		...
		if (window.innerWidth >= 850) {
			...
			if (scrollDown > 100 && scrollDown + windowHeight < documentHeight - 275) {
...
function setupFormValidation() {
	...
	clientEmailInput.addEventListener("input", function () {
		...
		isEmailValid = inputValue.trim() !== "" && emailPattern.test(inputValue);
		updateValidationState(clientEmailInput, isEmailValid);
		updateButtonState();
	});

	// Validation logic for Cellphone Number
	var clientCellphoneNumberInput = document.getElementById("clientCellphoneNumber");
	clientCellphoneNumberInput.addEventListener("input", function () {
		...
		updateValidationState(clientCellphoneNumberInput, isCellphoneNumberValid);
		updateButtonState();
	});
	...
	clientLotTypeSelect.addEventListener("change", function () {
		...
		isLotTypeValid = selectedValue === "freestandingLot" || selectedValue === "complexLot";
	...
	roofDirectionSelect.addEventListener("change", function () {
		...
		isRoofDirectionValid = selectedValue && validOptions.includes(selectedValue);
	...
	roofAngleInput.addEventListener("input", function () {
		...
		isRoofAngleValid = !isNaN(inputValue) && inputValue > 0 && inputValue <= 50;
	...
	totalResidentsInput.addEventListener("input", function () {
		...
		isTotalResidentsValid = !isNaN(inputValue) && inputValue > 0 && inputValue <= 100;
	...
	usageScheduleSelect.addEventListener("change", function () {
		...
		var validOptions = ["Day Time Usage", "Night Time Usage", "Balanced Usage"];
		isUsageScheduleValid = selectedValue && validOptions.includes(selectedValue);
